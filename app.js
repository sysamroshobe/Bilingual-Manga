...
function selectedIdsFromPayload(payload, orderedTitles) {
  const selected = [];
  const validIds = new Set((orderedTitles || []).map((t) => t?.enid).filter(Boolean));

  for (const key in payload) {
    if (!payload[key]) continue;
    const asIndex = Number(key);
    if (Number.isInteger(asIndex) && asIndex >= 0 && asIndex < orderedTitles.length) {
      const idFromIndex = orderedTitles[asIndex]?.enid;
      if (idFromIndex) selected.push(idFromIndex);
      continue;
    }
    if (validIds.has(key)) {
      selected.push(key);
...
function httpCdn(cdn) {
  return String(cdn).replace(/^https:\/\/cdn\.bilingualmanga\.org/i, 'http://cdn.bilingualmanga.org')
}

async function downloadCover(cdn, path) {
  const temp1 = await fetch(`${httpCdn(cdn)}/${path}`)
  if (!temp1.ok) {
    throw new Error(`HTTP ${temp1.status} for ${path}`)
  }
  const dest = decodeURIComponent(decodeURIComponent(path))
...
async function fetc(){
  ...
  for(let i in manga_tit)
  {
    ...
    if(!fs.existsSync(pat)){
      try {
        await downloadCover(cdn, pat)
      } catch (e) {
        console.warn(`Cover download skipped (${pat}):`, e.message)
      }
    }
    ...
    console.warn('fetc() failed:', e.message)
  }
}
