...
export async function load(event) 
{
    ...
    // Expose data availability so UI can show per-title indicators.
    const validIds = new Set(mangaData.map((row) => row?._id?.$oid).filter(Boolean))
    jsonc[0].manga_data_ids = Array.from(validIds)
